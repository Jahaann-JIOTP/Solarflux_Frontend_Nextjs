...
const StringTable = () => {
    ...
    const fetchData = async () => {
        ...
        try {
            ...
            if (rawData.length > 0) {
                const headerKeys = Object.keys(rawData[0]).filter((key) => key !== "Key");
    ...
    return (
        <div className="p-2">
            ...
            <div
                ...
                className="w-full h-[75vh] pt-[10px] mt-[20px] bg-[#0d2d42] p-5 rounded-lg mb-2 text-center shadow-[0px_0px_15px_rgba(0,136,255,0.7),_inset_0px_10px_15px_rgba(0,0,0,0.6)]"
            >
                {loading && (
                ...
                <div className={`!overflow-auto max-h-[70vh] mt-5 ${loading ? "hidden" : ""}`} id="main-section">
                    <table className="min-w-full text-[0.875rem] border border-collapse" >
                        <thead className="sticky top-0 z-10 bg-gradient-to-r from-[#0461A9] to-[#045A9E] text-white font-bold">
                            <tr>
                                <th className="p-4 border">Inverter</th>
                                <th className="p-4 border">MPPT</th>
                                <th className="p-4 border">String</th>
                                <th className="p-4 border">Watts/String</th>
                                {headers.map((date) => (
                                    <th key={date} className="p-4 border">{formatDate(date)}</th>
                                ))}
                            </tr>
                            <tr>
                                {["Inverter", "MPPT", "String", "Watts/String"].map((field) => (
                                    <th key={field} className="p-3 border">
                                        <select
                        ...
                        <tbody>
                            {filteredData.map((row) => (
                                <tr key={row.Key} className="text-center">
                                    <td className="p-4 border min-h-[50px]">{row.Inverter}</td>
                                    <td className="p-4 border min-h-[50px]">{row.MPPT}</td>
                                    <td className="p-4 border min-h-[50px]">{row.String}</td>
                                    <td className="p-4 border min-h-[50px]">{row.watts_per_string} W</td>
                                    {row.values.map((val) => (
                                        <td
                                            key={val.date}
                                            className="p-4 border min-h-[50px]"
