...
export default function ProductionComparison() {
  ...
  const fetchMpptIds = async () => {
    ...
    try {
      const response = await axios.post(
        config.BASE_URL + "production/get-mppt",
        {
  ...
  const createChart = async () => {
    ...
    try {
        ...
        const response = await axios.post("http://localhost:5000/analysis/aggregate-data1", {
            plant: selectedPlant,
        ...
        rawData.forEach((item) => {
            ...
            if (selectedResolution === "5min" || selectedResolution === "hourly") {
                timestampLabel = item.timestamp; // Directly use string timestamps
            } else if (selectedResolution === "daily") {
                timestampLabel = `${item.timestamp}`;
            } else if (selectedResolution === "weekly") {
                timestampLabel = `Week ${item.timestamp.week}, ${item.timestamp.year}`;
            } else if (selectedResolution === "monthly") {
                timestampLabel = `${item.timestamp.year}-${item.timestamp.month}`;
            } else if (selectedResolution === "quarter") {
                timestampLabel = `Q${item.timestamp.quarter}, ${item.timestamp.year}`;
            } else if (selectedResolution === "half-yearly") {
                timestampLabel = `H${item.timestamp.half}, ${item.timestamp.year}`;
            } else if (selectedResolution === "yearly") {
                timestampLabel = `${item.timestamp.year}`;
            }
        ...
        let dateAxis;
        if (["5min", "hourly"].includes(selectedResolution)) {
            dateAxis = chart.xAxes.push(new am4charts.DateAxis());
            dateAxis.baseInterval = {
                timeUnit: selectedResolution === "5min" ? "minute" : "hour",
                count: 1,
            };
        } else {
  ...
  return (
    ...
    <select
        ...
        onChange={(e) => {
            ...
            createChart(); // 🔥 Updates chart immediately
        }}
    >
      ...
      >
        ...
        <div
          ...
          className={`w-full h-[60vh] mt-[40px] ${loading ? "hidden" : ""}`}
        ></div>
      </div>
    </div>
  );
