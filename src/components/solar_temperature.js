...
const InverterTemperatureChart = ({ selectedPlant, dateRange }) => {
    const chartRef = useRef(null);
    const [option, setOption] = useState(2); // 1: Hourly, 2: Daily
    ...
    const renderChart = (snList, chartData) => {
      ...
      let chart = am4core.create('chartdivlayered2', am4charts.XYChart);
    ...
    return (
      ...
      ></div>
            <div id="chartdivlayered2" className={`w-full h-[30vh] ${loading ? "hidden" : ""}`}></div>
  ...
  export default InverterTemperatureChart;
